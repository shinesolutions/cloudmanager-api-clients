  ...
  class Variable
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AemCloudManagerApiClient::Variable` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AemCloudManagerApiClient::Variable`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/[a-zA-Z_][a-zA-Z_0-9]*/)
      if !@name.nil? && @name !~ pattern
        invalid_properties.push("invalid value for \"name\", must conform to the pattern #{pattern}.")
      end

      if !@value.nil? && @value.to_s.length > 2048
        invalid_properties.push('invalid value for "value", the character length must be smaller than or equal to 2048.')
      end
    ...
    def valid?
      ...
      return false if !@name.nil? && @name !~ Regexp.new(/[a-zA-Z_][a-zA-Z_0-9]*/)
      return false if !@value.nil? && @value.to_s.length > 2048
      return false if !@value.nil? && @value.to_s.length < 0
      type_validator = EnumAttributeValidator.new('String', ["string", "secretString"])
    ...
    def value=(value)
      if !value.nil? && value.to_s.length > 2048
        fail ArgumentError, 'invalid value for "value", the character length must be smaller than or equal to 2048.'
      end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["string", "secretString"])
