%% basic handler
-module(openapi_pipeline_execution_handler).
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'AdvancePipelineExecution'
    }
) ->
    {[<<"PUT">>], Req, State};
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'CancelPipelineExecutionStep'
    }
) ->
    {[<<"PUT">>], Req, State};
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'GetCurrentExecution'
    }
) ->
    {[<<"GET">>], Req, State};
...
valid_content_headers(
    ...
    Headers = ["x-gw-ims-org-id","Authorization","x-api-key","Content-Type"],
...
valid_content_headers(
    ...
    Headers = ["x-gw-ims-org-id","Authorization","x-api-key"],
    {Result, Req} = validate_headers(Headers, Req0),
    {Result, Req, State};
...
valid_content_headers(
    ...
    Headers = ["Accept","x-gw-ims-org-id","Authorization","x-api-key"],
    {Result, Req} = validate_headers(Headers, Req0),
    {Result, Req, State};
