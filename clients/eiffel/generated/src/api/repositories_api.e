...
feature -- API Access
	...
	repositories (program_id: STRING_32; x_gw_ims_org_id: STRING_32; authorization: STRING_32; x_api_key: STRING_32): detachable REPOSITORY_LIST
		...
		do
			...
			l_path := "/api/program/{programId}/repositories"
			...
			elseif attached { REPOSITORY_LIST } l_response.data ({ REPOSITORY_LIST }) as l_data then
				Result := l_data
			else
	...
	repository (program_id: STRING_32; repository_id: STRING_32; x_gw_ims_org_id: STRING_32; authorization: STRING_32; x_api_key: STRING_32): detachable REPOSITORY
			...
			-- argument: program_id Identifier of the program (required)
			-- 
			-- argument: repository_id Identifier of the repository (required)
			-- 
			-- argument: x_gw_ims_org_id IMS organization ID that the request is being made under. (required)
			-- 
			-- argument: authorization Bearer [token] - An access token for the technical account created through integration with Adobe IO (required)
		...
		do
			...
			l_path := "/api/program/{programId}/repository/{repositoryId}"
			l_path.replace_substring_all ("{"+"programId"+"}", api_client.url_encode (program_id.out))
			l_path.replace_substring_all ("{"+"repositoryId"+"}", api_client.url_encode (repository_id.out))
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { REPOSITORY } l_response.data ({ REPOSITORY }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
