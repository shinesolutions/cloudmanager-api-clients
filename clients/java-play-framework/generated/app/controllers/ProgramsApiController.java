...
public class ProgramsApiController extends Controller {

    private final ProgramsApiControllerImpInterface imp;
    private final ObjectMapper mapper;
    private final Configuration configuration;

    @Inject
    private ProgramsApiController(Configuration configuration, ProgramsApiControllerImpInterface imp) {
        this.imp = imp;
    ...
    @ApiAction
    public Result deleteProgram(String programId) throws Exception {
        ...
        Program obj = imp.deleteProgram(programId, xGwImsOrgId, authorization, xApiKey);
    ...
    @ApiAction
    public Result getProgram(String programId) throws Exception {
        String valuexGwImsOrgId = request().getHeader("x-gw-ims-org-id");
        String xGwImsOrgId;
        if (valuexGwImsOrgId != null) {
            xGwImsOrgId = valuexGwImsOrgId;
        ...
        Program obj = imp.getProgram(programId, xGwImsOrgId, authorization, xApiKey);
        if (configuration.getBoolean("useOutputBeanValidation")) {
            OpenAPIUtils.validate(obj);
        }
        JsonNode result = mapper.valueToTree(obj);
    ...
    @ApiAction
    public Result getPrograms() throws Exception {
        String valuexGwImsOrgId = request().getHeader("x-gw-ims-org-id");
        String xGwImsOrgId;
        if (valuexGwImsOrgId != null) {
            xGwImsOrgId = valuexGwImsOrgId;
        ...
        ProgramList obj = imp.getPrograms(xGwImsOrgId, authorization, xApiKey);
        if (configuration.getBoolean("useOutputBeanValidation")) {
            OpenAPIUtils.validate(obj);
        }
        JsonNode result = mapper.valueToTree(obj);
