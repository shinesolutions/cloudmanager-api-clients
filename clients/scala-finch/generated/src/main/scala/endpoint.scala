...
object endpoint {
  ...
  */
  def makeService(da: DataAccessor): Service[Request, Response] = (
          BranchesApi.endpoints(da)  :+:
          EnvironmentsApi.endpoints(da)  :+:
          PipelineExecutionApi.endpoints(da)  :+:
          PipelinesApi.endpoints(da)  :+:
          ProgramsApi.endpoints(da)  :+:
          RepositoriesApi.endpoints(da)  :+:
          VariablesApi.endpoints(da) 
  ).handle({
